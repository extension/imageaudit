...
class HostedImage < ActiveRecord::Base
  ...
  def self.bulk_change_stock_and_staff_review(image, contributor)
    @currentcontributor = Contributor.find(contributor)
    @image = HostedImage.find(image)
     #change stock review
    previous_value = @image.is_stock
    @image.update_attributes({is_stock: 0, is_stock_by: @currentcontributor.id, staff_reviewed: 1, staff_reviewed_by: @currentcontributor.id})
    ...
    @image.update_attributes({staff_reviewed: 'yes', staff_reviewed_by: @currentcontributor.id})
    AuditLog.create(contributor: @currentcontributor,
                    auditable: @image,
                    changed_item: 'staff_reviewed',
                    previous_check_value: previous_value,
